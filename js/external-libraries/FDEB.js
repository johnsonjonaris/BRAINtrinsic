    ...
    d3.ForceEdgeBundling = function () {
        ...
        function compute_divided_edge_length(e_idx) {
            ...
            for (var i = 1; i < subdivision_points_for_edge[e_idx].length; i++) {
                segment_length = euclidean_distance(subdivision_points_for_edge[e_idx][i], subdivision_points_for_edge[e_idx][i - 1]);
        ...
        function project_point_on_line(p, Q) {
            ...
            var L = Math.pow((Q.target.x - Q.source.x),2) + Math.pow((Q.target.y - Q.source.y),2) + Math.pow((Q.target.z - Q.source.z),2);
            var r = ((p.x - Q.source.x) * (Q.target.x - Q.source.x) + (p.y - Q.source.y) * (Q.target.y - Q.source.y) + (p.z - Q.source.z) * (Q.target.z - Q.source.z)) / L;
        ...
        function apply_resulting_forces_on_subdivision_points(e_idx, P, S) {
            ...
            for (var i = 1; i < P + 1; i++) { // exclude initial end points of the edge 0 and P+1
                ...
                resulting_force.z = S * (spring_force.y + electrostatic_force.z);
